...
Graders are classes with grade(action, state) -> float method
Scores always strictly between 0 and 1
"""
...
class EasyGrader:
    ...
    def grade(self, action, state) -> float:
        """Grade an easy ticket task"""
        history = state.get("history", []) if state else []
        state_data = state if state else {}
...
def grade_easy(history, state_data=None):
    """Legacy function wrapper - use EasyGrader class instead"""
    grader = EasyGrader()
    state = {"history": history, **(state_data or {})}
    return grader.grade(None, state)


def grade_medium(history, state_data=None):
    """Legacy function wrapper - use MediumGrader class instead"""
    grader = MediumGrader()
    state = {"history": history, **(state_data or {})}
    return grader.grade(None, state)
...
def grade_hard(history, state_data=None):
    """Legacy function wrapper - use HardGrader class instead"""
    grader = HardGrader()
    state = {"history": history, **(state_data or {})}
    return grader.grade(None, state)